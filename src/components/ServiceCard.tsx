...
export const additionalServiceTaglines: Record<string, string> = {
  ...
  'hr-payroll': 'Workforce efficiency',
  'tax-compliance': 'Stay compliant',
  'system-audits': 'Risk assessment',
};
...
export default function ServiceCard({
  ...
  return (
    <article className="group relative bg-white rounded-xl border border-[#e8ecf1] p-7 sm:p-8 shadow-[0_4px_24px_rgba(24,25,28,0.04)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(24,25,28,0.1)] hover:border-golden/40 overflow-hidden h-full flex flex-col">
      ...
      <p className="text-[#6f7174] text-[15px] leading-[28px] mb-6 line-clamp-4 flex-1">
        {description}
      </p>

      <Link
        to={href}
        className="inline-flex items-center gap-2 text-charcoal font-semibold text-[13px] uppercase tracking-wider group/link hover:text-golden transition-colors mt-auto"
      >
        {linkLabel}
